require([
    ...
    window.xpanseValidator = function (endpoint, environmentId, skip = false) {
        ...
        if (errors.length > 0) {
            alert({
                title: $t('xpanse Credential Validation Failed'),
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success === 'true') {
                    ...
                    alert({
                        title: $t('xpanse Credential Validation Failed'),
                        content: $t('Your xpanse Credentials could not be validated. Please ensure you have selected the correct environment and entered a valid Public Key and Private Key.')
                    });
                }
            }
        }).always(function () {
